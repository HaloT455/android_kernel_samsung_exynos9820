...
#include <limits.h>
#define unlikely(x) (x)
#define cpufreq_can_do_remote_dvfs(policy) true
#include <stdio.h>
#include <string.h>

typedef uint64_t u64;
typedef int64_t s64;
struct cpufreq_policy { bool fast_switch_enabled; unsigned int cur; int rwsem; };
...
#define irq_work_queue_on(work, cpu) irq_work_queue(work)
...
static void kthread_queue_work(struct kthread_worker *worker,
                              struct kthread_work *work)
{
    work_calls++;
    work_pending = true;
...
int main(void)
{
    ...
    /* Pending driver work must not prevent sampling a newer demand. */
    reset();
    state.work_in_progress = true;
    state.min_rate_limit_ns = 2000000;
    state.last_freq_update_time = 10000000;
    assert(!sugov_should_update_freq(&state, 11000000));
    assert(sugov_should_update_freq(&state, 12000000));
    state.need_freq_update = true;
    assert(sugov_should_update_freq(&state, 11000000));
    assert(state.next_freq == UINT_MAX && !state.need_freq_update);
...
def main():
    ...
    names = ("sugov_should_update_freq", "sugov_up_down_rate_limit", "sugov_update_commit",
